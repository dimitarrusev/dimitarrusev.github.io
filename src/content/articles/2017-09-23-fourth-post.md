...
The final thing we need to do before we can get going with the router is add the `base` tag to our `index.markup` file.
...
It's super easy to define this, open up your `index.markup` and add the `base` tag into the header:
...
For more, check out the official [Angular docs](https://angular.io/docs/ts/latest/guide/router.markup) on the router.
